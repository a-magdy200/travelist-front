...
function getComparator<Key extends keyof IBookingInterface>(
	order: Order,
	orderBy: Key
): (a: IBookingInterface, b: IBookingInterface) => number {
	return order === 'desc'
...
const headCells: readonly HeadCellInterface<IBookingInterface>[] = [
	{
		id: 'id',
		numeric: true,
...
interface EnhancedTableProps {
	...
	onRequestSort: (
		...
		property: keyof IBookingInterface
	) => void
...
function EnhancedTableHead(props: EnhancedTableProps) {
	...
	const createSortHandler =
		(property: keyof IBookingInterface) => (event: React.MouseEvent<unknown>) => {
...
const ListBookingComponent=({bookings}:IBookingListProps)=>{
    const [order, setOrder] = useState<Order>('asc')
	const [orderBy, setOrderBy] = useState<keyof IBookingInterface>('id')
	const [selected, setSelected] = useState<readonly string[]>([])
	const [page, setPage] = useState(0)
	const [rowsPerPage, setRowsPerPage] = useState(10)

    const removeBooking = async (id: number | undefined) => {
		if (window.confirm('Are you sure?')) {
			console.log(id)
			try {
				const response: IResponseInterface<IBookingInterface> =
					await api<IBookingInterface>({
						url: `/api/cycles/booking/delete/${id}`,
						method: 'DELETE',
					})
        ...
        <div>
            {
             bookings?
             <div>
                <Box className="listBookings" sx={{ width: '97%' }}>
								...
								<Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
									...
									<TableBody>
										{stableSort(bookings, getComparator(order, orderBy))
											...
											.map((booking: IBookingInterface, index) => {
												return (
													<TableRow>
														<TableCell align="center">{booking.id}</TableCell>
														<TableCell align="center">{booking.cycle.name}</TableCell>
														<TableCell align="center">
															{booking.travelers.user.name}
														</TableCell>
														...
														<TableCell align="center">
															<NavLink to={`/booking/show/${booking.id}`}>
																<Button
