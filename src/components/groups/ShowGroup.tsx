...
const ShowGroupComponent = ({ group }: IGroupShowProps) => {
  const photoPath = `${config.apiUrl}/${group?.cover_picture}`;
	const [followersCount, setFollowersCount] = useState(group?.followers_count || 0);
	const { user } = useAuth();

	const [isFollowing, setIsFollowing] = useState<boolean>(!!group?.followers?.length && !!group.followers.find(({ id }) => id === user.id))
